...
This package adds jalali support to filament `DatePicker` and `DateTimePicker`.
...
## Upgrading
### v1 to v2
- step 1:Change the package version in you `composer.json` to `^3` and run the composer update command:
```
...
- step 2 (optional):
    ...
    You can change `jalaliDatePicker` and `jalaliDateTimePicker` to the default `DatePicker` and `DateTimePicker` and then use `jalali()` method to convert a datepicker to jalali format
For example:
```php
//convert
...
\Filament\Forms\Components\DatePicker::make('birth_date')->jalali()
```
The old format will work in `v3` but we might remove it in the next major versions.
## Contributing
